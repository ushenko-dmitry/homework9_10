...
public class UserTableServiceImpl implements TableService {
    ...
    public boolean createTable() {
        try (Connection connection = ConnectionPoolImpl.getInstance().getConnection()) {
            ...
            try {
                tableRepository.executeQuery(connection, propertyUtil.getProperty(SQL_REQUEST_CREATE_TABLE_USER));
                connection.commit();
                return true;
    ...
    public boolean removeTable() {
        try (Connection connection = ConnectionPoolImpl.getInstance().getConnection()) {
            connection.setAutoCommit(false);
            try {
                tableRepository.executeQuery(connection, propertyUtil.getProperty(SQL_REQUEST_DROP_TABLE_USER));
                connection.commit();
                return true;
            } catch (SQLException ex) {
                logger.error(ex.getMessage(), ex);
